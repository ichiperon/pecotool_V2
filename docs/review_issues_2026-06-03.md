...
| --- | --- | --- | --- | --- |
| PCT-049 | P1 | `pdfMetadataLoader` の `isValidBBoxMetaRecord` が all-or-nothing で、保存メタに不正エントリ（範囲外confidence/負・小数order/NaN bbox）が**1件**混じると `loadPecoToolBBoxMeta` が**文書全体の meta を null** にし、**全ページのOCRデータが再オープンで消失**。最優先保証「他の正常ページを巻き込まない」に反する。「んなわけあるか」級異常系テスト（まつり）で発覚 | まつり発見 / シオン / マリン | loader を `sanitizeBBoxMetaRecord` に刷新（不正エントリのみdrop・有効分は必ず保持・DANGEROUS_KEYはキー単位skip・構造破綻時のみnull）。保存経路は不変。pathological/単体/save系 60 passed; tsc 0; reviewer LGTM。有効データは挙動不変 |
| PCT-048 | P2 | 低信頼度ハイライトが粗いヒューリスティック(0.3/0.5/0.9)依存で実用性が低く、バッジがトグル無関係に出て「押しても変わらない」。表記も実態とズレ。ユーザー指示で「明らかにおかしいものだけ」へ厳格化 | フブキ / マリン | confidence スコア非依存の `blockQuality.ts`（空テキスト ∪ BB重なり intersection/min面積≥0.5）へ刷新。バッジ/ハイライト両方をトグル連動、ラベル「要確認マーク」へ。保存経路は不変（reviewer 断定）。blockQuality+pdfCanvasStaticLayer 55 passed; tsc 0; reviewer LGTM |

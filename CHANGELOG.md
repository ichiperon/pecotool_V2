...
> **回転表示の修正・ページ操作の高速化** — 実機確認で見つかった「UIでページ回転しても画面に反映されない」既存バグの修正と、ページ削除・ページ送りの体感速度改善。多観点レビューで検出した P0 2件（回転時のBB位置ズレ／キャッシュ全ミス）も修正済み。広域 vitest 2147 passed。

### Fixed

- **UIでページを回転しても画面表示が回転しない問題を修正** (PCT-120): ページ回転（90/180/270）の状態と保存PDFには回転が反映されていたが、画面プレビューが回転を無視していた（#207実装当初からの既存バグ）。pdfjs viewport に回転を合成し、バウンディングボックスの枠・ハイライト・マウス操作の当たり判定も回転座標に追従させた。回転0°（無回転）の描画・操作は完全に従来どおりで変化なし。
- **bitmapページキャッシュが常時ミスしていた問題を修正** (PCT-119): 回転対応でキャッシュキー末尾に付けた回転値の書式が、キャッシュのキー解析（末尾を数値として扱う）と噛み合わず、回転を使わない通常のページ送りでも毎回ページを再描画していた。キー書式を修正し、再訪ページは即時表示に復帰。
- **回転時にBB枠がテキストからずれる不具合を修正** (PCT-120, レビュー指摘): 縦横比が異なるページを90/270度回転したとき、座標変換の縦横寸法の取り違えでBB枠・ハイライト・当たり判定がページ画像とずれていた。保存側の回転（pdfSaverCore）と一致するよう修正し、回帰テストを正しい期待値に是正。

### Performance / UX

- **ページ削除の高速化** (PCT-121): ページ削除・並べ替えのたびに全ページのサムネイルを破棄して再生成していたのを、変化したページ分だけ無効化し生き残りページのサムネイルを再利用する差分方式に変更。大量ページのPDFほど削除が軽くなる。
- **ページ送りの遅延短縮（固定ズーム時）** (PCT-122): 自動フィット表示を解除して固定ズームで使っているとき、ページ切替時の固定待ち時間（フィット確定待ちの50ms）を省いて即時描画するようにした。自動フィット表示中は従来どおり。
...
- **OCRテキスト層の位置補正の既定を 0/0 に変更** (PCT-117): 既定が「右4mm・下2mm」のONになっており、保存PDFの透明テキスト層がツール表示のBB枠から常時ずれていた。位置補正は素の座標平行移動（ビューア非依存）で、補正値0/0のときのみ表示BBと一致する。既定を0/0へ変更し、ツール表示のBB位置と保存テキスト層を一致させた。補正機能はopt-inで存続。**アプリ更新時、設定の位置補正値は0/0にリセットされます**（必要な場合はOCR設定で再設定してください）。

### Fixed

- **全ページ位置補正適用が1ページの抽出失敗で保存全体を中断する問題を修正** (PCT-106): 失敗ページはフォールバックして他ページの保存を継続し、失敗をトーストで可視化。失敗ページは元PDFのテキスト層を温存する。
- **保存処理中のUndo/RedoでOCR編集の一時退避キーがずれる問題を修正** (PCT-108): 保存の待機中に並べ替えを含むUndoが走るとライブのページ順を遅延参照してIDBキーが保存スナップショットと乖離していた。操作時点のページ順を固定して書き込むよう統一。
- **位置補正のバウンディングボックス再描画で信頼度が一時的に失われる問題を修正** (PCT-112): 再描画経路で信頼度を引き継ぎ、低信頼ハイライトが保存往復で一時消失しないようにした。

### Added

- **位置補正の回転/縦書きの方向検証テストを追加** (PCT-107): ページ回転（0/90/180/270度）と縦書き/横書きのすべてで、位置補正が表示上の右下方向へ一貫してずれることを確証（逆転なし）。

### Performance / UX

- **全ページ位置補正適用の進捗表示と動的タイムアウト** (PCT-109): 進捗トーストを表示し、固定600秒のタイムアウトをページ数比例（下限600秒）に変更してフリーズ誤認と崖を解消。

### Security / Robustness

- 位置補正offsetを±20mmにclamp（PCT-110）／一時ファイル直書きに500MB上限と起動時クリーンアップを追加（PCT-116）／保存チャンク書込先パスの自己完結検証（PCT-113）。

### 既知の注意

- ページの並べ替えを未保存のまま本バージョンへ更新すると、更新直後の初回のみ一時退避した編集が1ページ分ずれて復元される可能性があります（PCT-115）。**更新前に保存することを推奨します。**

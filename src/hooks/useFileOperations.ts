 ...
 *
 * Rust 側は std::io::Error::to_string() の英文メッセージか
 * `format!("write failed: {}", e)` を返してくる。コード番号 (os error 32) や
 * 英語フレーズの両方で検出できるように緩めの正規表現でマッチする。
 */
function isWriteAccessError(message: string): boolean {
...
function ensurePrefetchOriginalBytes(filePath: string): Promise<Uint8Array | null> {
  ...
  const run = async (): Promise<Uint8Array | null> => {
    try {
      ...
      // ファイル切替が起きていても (現在開いている doc.filePath != filePath)、
      // この prefetch を発火した時点ではキャッシュ対象なので格納する。
...
export function useFileOperations(
  ...
  // executeSaveAs は下で定義されるため、_executeSave から参照できるよう ref で間接化する。
  // issue #53: writeFileAtomically が EACCES/EBUSY で失敗したときに showToast の
  // action ボタンから「別名で保存」へフォールバックさせるのに使う。

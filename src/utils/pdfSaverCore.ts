 ...
 * を使い、(originalRotation→user→finalRotation) を全 (Ro, delta) 組み合わせで
 * 検証・一般化したもの。
 *
...
export async function buildPdfDocumentCore(
    ...
    for (let pi = 0; pi < pdfDoc.getPageCount(); pi++) {
      ...
      const repairBlocks = entries
        ...
        .map((block) => {
          ...
          // #337: confidence を repair 経路でも引き継ぐ。
          // 旧注釈「次回保存時に再永続化されるため一時的欠落は許容」は実態と乖離していた。
          // bloat repair は textBlocks を existingBBoxMeta から復元するため、confidence が
          // ここで欠落すると再オープン後の低信頼ハイライトが消える。
          if (typeof (block as { confidence?: unknown }).confidence === 'number') {
            out.confidence = (block as { confidence: number }).confidence;
          }
          return out;
        });
  ...
  for (const [pageIndex, pageData] of pageEntriesToWrite) {
    ...
    // 元 /Rotate≠0 のページ（スキャナ由来）でプレビューと食い違う問題があった。
    ...
    // 保存後の再ロード時に pdfjs が finalRotation viewport で解釈するよう、
    // remapBboxForRotation で座標を変換してから書き込む。
    // userRotation が undefined のページは remapBboxForRotation(delta=0) が恒等を返す。
    ...
    // #352: cm 構築は捕捉フレーム (originalRotation) を使う。bbox が originalRotation 空間
    // の座標なので、cm も同フレームで構築しなければテキスト層の位置が合わない。

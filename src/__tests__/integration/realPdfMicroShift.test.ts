...
describe.skipIf(!hasRealPdf)('REAL PDF: 全 OCR 微移動 → 別名保存 → 差分確認', () => {
  ...
  it('20〜30 BB ページの中央 BB を分割 → 別名保存 → 再読込 meta が N+1 件で対応関係がずれない', async () => {
    ...
    // --- 複数ブロック (>=10) を持つ水平書きページを探す ---
    //   (PDF により密度が異なるため、元の 20-30 BB 条件は緩和)
    let targetPageIndex = -1;
    for (let i = 0; i < totalPages; i++) {
      const pd = doc.pages.get(i)!;
      const count = pd.textBlocks.length;
      if (count >= 10 && pd.textBlocks.every((b) => b.writingMode === 'horizontal')) {
        targetPageIndex = i;
        break;
      }
    }
    if (targetPageIndex === -1) {
      // 横書き制約も緩和
      for (let i = 0; i < totalPages; i++) {
        if (doc.pages.get(i)!.textBlocks.length >= 10) { targetPageIndex = i; break; }
      }
    }

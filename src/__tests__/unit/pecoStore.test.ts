...
describe('pecoStore', () => {
  ...
  // ── issue #196: searchTerm / searchHitIndex ───────────────────
  describe('issue #196: searchTerm / searchHitIndex', () => {
    it('初期状態では searchTerm が空文字で searchHitIndex が 0', () => {
      expect(usePecoStore.getState().searchTerm).toBe('')
      expect(usePecoStore.getState().searchHitIndex).toBe(0)
    })

    it('setSearchTerm で searchTerm が更新され searchHitIndex が 0 にリセットされる', () => {
      usePecoStore.setState({ searchHitIndex: 3 })
      usePecoStore.getState().setSearchTerm('hello')

      const s = usePecoStore.getState()
      expect(s.searchTerm).toBe('hello')
      expect(s.searchHitIndex).toBe(0)
    })

    it('setSearchTerm で空文字を渡すと searchTerm が空になる', () => {
      usePecoStore.setState({ searchTerm: 'foo', searchHitIndex: 2 })
    ...
    it('nextSearchHit で searchHitIndex が 1 進む', () => {
      usePecoStore.setState({ searchHitIndex: 0 })
      usePecoStore.getState().nextSearchHit(5)

      expect(usePecoStore.getState().searchHitIndex).toBe(1)
    })

    it('nextSearchHit は cyclic: 末尾から先頭に戻る', () => {
      usePecoStore.setState({ searchHitIndex: 4 })
      usePecoStore.getState().nextSearchHit(5)

      expect(usePecoStore.getState().searchHitIndex).toBe(0)
    })

    it('nextSearchHit で totalHits=0 なら何もしない', () => {
    ...
    it('prevSearchHit で searchHitIndex が 1 戻る', () => {
      usePecoStore.setState({ searchHitIndex: 3 })
      usePecoStore.getState().prevSearchHit(5)

      expect(usePecoStore.getState().searchHitIndex).toBe(2)
    })

    it('prevSearchHit は cyclic: 先頭から末尾に戻る', () => {
      usePecoStore.setState({ searchHitIndex: 0 })
      usePecoStore.getState().prevSearchHit(5)

      expect(usePecoStore.getState().searchHitIndex).toBe(4)
    })

    it('prevSearchHit で totalHits=0 なら何もしない', () => {
      usePecoStore.setState({ searchHitIndex: 1 })
